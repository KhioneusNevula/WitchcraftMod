...
public class BlockSieve extends BlockBase {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		MagicalSieve wire = (MagicalSieve)worldIn.getTileEntity(pos);
		if (!playerIn.isSneaking()) {
			System.out.println("Side cycled to " + wire.cycle(facing));
		} else {
			wire.setRemoveOnly(!wire.isRemoveOnly());
			System.out.println("Remove only cycled to " + wire.isRemoveOnly());
		}
		return true;
	}
